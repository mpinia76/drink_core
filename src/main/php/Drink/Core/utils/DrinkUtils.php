...
class DrinkUtils {
	...
	const DRINK_CONCEPTO_GASTO_VARIOS = 1;

	const DRINK_CONCEPTO_MOVIMIENTO_GASTO= 2;
	const DRINK_CONCEPTO_MOVIMIENTO_ANULACIONGASTO= 3;
	const DRINK_CONCEPTO_MOVIMIENTO_VENTA= 4;
	const DRINK_CONCEPTO_MOVIMIENTO_ANULACIONVENTA= 5;
	const DRINK_CONCEPTO_MOVIMIENTO_PAGO= 6;
	const DRINK_CONCEPTO_MOVIMIENTO_DEVOLUCION= 7;
	const DRINK_CONCEPTO_MOVIMIENTO_ACTUALIZACION= 8;
	const DRINK_CONCEPTO_MOVIMIENTO_COMISIONVENTA= 9;
	...
	const DRINK_ID_PORCENTAJE_PRECIO_LISTA = 2;
	...
	public static function sendMailPop($nameTo, $to, $subject, $msg, $attachs) {
		...
		/*$libsHome= DrinkConfig::getInstance()->getLibsPath();
		...
		if( DrinkConfig::TEST_MODE ){
			//me los envÃ­o todos a mi en test
			$to = DrinkConfig::TEST_MAIL_TO;
		}
		...
		$mail->Host = DrinkConfig::POP_MAIL_HOST;
		//Set the SMTP port number - likely to be 25, 465 or 587
		$mail->Port = DrinkConfig::POP_MAIL_PORT;
		$mail->SMTPSecure = DrinkConfig::POP_MAIL_SMTP_SECURE;
		//Whether to use SMTP authentication
		$mail->SMTPAuth = true;
		//Username to use for SMTP authentication
		$mail->Username = DrinkConfig::POP_MAIL_USERNAME;
		//Password to use for SMTP authentication
		$mail->Password = DrinkConfig::POP_MAIL_PASSWORD;

		//Set who the message is to be sent from
		$mail->setFrom(DrinkConfig::MAIL_FROM, DrinkConfig::MAIL_FROM_NAME);
		//Set an alternative reply-to address
		$mail->addReplyTo(DrinkConfig::MAIL_FROM, DrinkConfig::MAIL_FROM_NAME);
	...
	public static function getConceptoGastoVarios(){
		...
		return ServiceFactory::getConceptoGastoService()->get( self::DRINK_CONCEPTO_GASTO_VARIOS );
	}

	public static function getConceptoMovimientoGasto(){

		return ServiceFactory::getConceptoMovimientoService()->get( self::DRINK_CONCEPTO_MOVIMIENTO_GASTO );
	}

	public static function getConceptoMovimientoAnulacionGasto(){

		return ServiceFactory::getConceptoMovimientoService()->get( self::DRINK_CONCEPTO_MOVIMIENTO_ANULACIONGASTO );
	}

	public static function getConceptoMovimientoVenta(){

		return ServiceFactory::getConceptoMovimientoService()->get( self::DRINK_CONCEPTO_MOVIMIENTO_VENTA );
	}

	public static function getConceptoMovimientoAnulacionVenta(){

		return ServiceFactory::getConceptoMovimientoService()->get( self::DRINK_CONCEPTO_MOVIMIENTO_ANULACIONVENTA );
	}

	public static function getConceptoMovimientoPago(){

		return ServiceFactory::getConceptoMovimientoService()->get( self::DRINK_CONCEPTO_MOVIMIENTO_PAGO);
	}
	...
	public static function getCuentaUnica(){
		...
		return ServiceFactory::getCuentaService()->get( self::DRINK_CUENTA_UNICA );
	}

	public static function getCajaDeafault(){

		return ServiceFactory::getCajaService()->get( self::DRINK_CAJA_DEFAULT );
	}

	public static function getPrecioDolar(){

		return ServiceFactory::getParametroService()->get( self::DRINK_ID_DOLAR );
	}

	public static function getPorcentajePrecioLista(){

		return ServiceFactory::getParametroService()->get( self::DRINK_ID_PORCENTAJE_PRECIO_LISTA );
	}

	public static function getTipoProductoDefault(){

		return ServiceFactory::getTipoProductoService()->get( self::DRINK_TIPO_PRODUCTO_DEFAULT );
	}
